...
void setBeacon() {
  ...
  //EddystoneTLM.setCount(advCount++); //messages sent since start 
  //EddystoneTLM.setTime((uint32_t) millis()); //time in milliseconds sinse start 
  ...
  Serial:printf("ServiceData Length: %d", EddystoneTLM.getData().length());
